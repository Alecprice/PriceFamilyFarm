...
export async function pushLocalFarmStores(endpoint, token) {
  ...
  const result = { uploaded: 0, alreadyCurrent: 0, conflicts: [], skipped: 0 };
  ...
  for (const store of FARM_STORES) {
    ...
    try {
      ...
      if (response.status === 409) {
        const serverRevision = Number(body?.revision);
        const serverSchemaVersion = Number(body?.schemaVersion);
        const serverChecksum =
          typeof body?.checksum === "string"
            ? body.checksum.trim().toLowerCase()
            : "";
        const serverPayloadValid = validFarmStoreValue(store, body?.payload);
        const serverPayloadChecksum = serverPayloadValid
          ? await checksum(body.payload)
          : "";
        ...
        if (
          ...
          CHECKSUM_PATTERN.test(serverChecksum) &&
          serverChecksum === requestBody.checksum &&
          serverChecksum === serverPayloadChecksum
        ) {
          revisions[store.id] = serverRevision;
          writeRevisionMap(revisions);
        ...
        result.conflicts.push({
          ...
          label: store.label,
          serverRevision:
            Number.isInteger(serverRevision) && serverRevision >= 0
              ? serverRevision
              : 0,
        });
        continue;
