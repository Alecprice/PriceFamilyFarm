...
const checks = [
  ...
  ["Playwright cache restore uses pinned actions/cache", "actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9 # v6.1.0"],
  ["Playwright cache restore has an id", "id: playwright_cache"],
  ...
  ["Playwright cache save uses pinned actions/cache", "actions/cache/save@55cc8345863c7cc4c66a329aec7e433d2d1c52a9 # v6.1.0"],
  ["Playwright cache save skips exact cache hits", "steps.playwright_cache.outputs.cache-hit != 'true'"],
  ["Playwright cache save reuses the restore primary key", "key: ${{ steps.playwright_cache.outputs.cache-primary-key }}"],
...
if (workflow.includes("save-always:")) {
  failures.push("deprecated actions/cache save-always input is absent");
}

const checkCount = checks.length + 1;
if (failures.length) {
  for (const failure of failures) console.error(`FAIL: ${failure}`);
  console.error(`Quality workflow verification failed: ${failures.length}/${checkCount} checks failed.`);
  process.exit(1);
}

console.log(`Quality workflow verification passed: ${checkCount} checks.`);
