...
test("farm journal persists browser-local entries and filters them", async ({ page }) => {
  ...
  await page.getByLabel("Title").fill("Irrigation repair");
  await page.getByLabel("Category").selectOption("Maintenance");
  await page.getByLabel("Observation / note").fill("Replaced a leaking fitting and checked the line.");
  ...
  await page.getByLabel("Category filter").selectOption("Market");
